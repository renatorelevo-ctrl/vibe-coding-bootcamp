...
### 2. Optional: Kimi K3 als eigenständiger Worker (Upgrade)

Mit einer Kimi Code Membership (Moderato, ~$19/Monat) übernimmt Kimi K3
den Worker-Slot komplett. Vorteil: Worker und Checker teilen sich dann
kein Budget mehr, und ein echter Fremdanbieter prüft OpenAI-fremden Code.

1. Kimi Code Membership abschließen (Neuabos sind kapazitätsbedingt oft
   nur schubweise offen), in der **Kimi Code Console** einen API-Key
   erstellen (nicht platform.moonshot.ai — das ist die teure
   Pay-as-you-go-API).
2. `cp .env.devteam.example .env.devteam`, `KIMI_API_KEY` eintragen und
   `WORKER_PROVIDER="kimi"` setzen.

### 3. Optional: DeepSeek als Fallback-Worker

Für den Fall, dass das Limit des aktiven Workers mal erreicht ist:
...
2. In `.env.devteam` den Key eintragen und bei Bedarf
...
| OpenAI-/Kimi-Limit erreicht | `WORKER_PROVIDER="deepseek"` in `.env.devteam` (Schritt 3) |
| `KIMI_API_KEY fehlt` / Kimi-Auth-Fehler | Key aus der Kimi Code Console verwenden, nicht von platform.moonshot.ai; Endpoint-Details ggf. in der Console-Doku prüfen |
| `DEEPSEEK_API_KEY fehlt` | `.env.devteam` anlegen/befüllen (Schritt 3) |
